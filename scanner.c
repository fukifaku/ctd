...
void skipComment() {
  // TODO
  int state = 0;
  if (charCodes[currentChar-1] == CHAR_TIMES)
  {
...
Token* readConstString(void){
  ...
  int count = 1;
  token->conststring = (char *)malloc(sizeof(char));
  readChar();
  if (currentChar == EOF)
  {
    token->tokenType = TK_NONE;
  ...
  token->conststring[0] = (char) currentChar;
  readChar();
  if (currentChar == EOF)
  {
    token->tokenType = TK_NONE;
  ...
  while (currentChar != EOF && charCodes[currentChar] != CHAR_DOUBLEQUOTE){
    token->conststring = (char *) realloc(token->conststring, (count+1) * sizeof(char));
    token->conststring[count++] = (char) currentChar;
    readChar();
  }
  token->conststring = (char *) realloc (token->conststring, (count+1) * sizeof(char));
  token->conststring[count+1] = '\0';
